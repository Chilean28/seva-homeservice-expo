...
async function ensureUserProfileRow(user: User) {
  ...
  const ut = meta.user_type === 'worker' || meta.user_type === 'customer' ? meta.user_type : UserType.WORKER;
...
/** Do not pass `options.data` to signInWithOtp — that forces link email instead of OTP. */
...
/** Call after session is set from email confirmation deep link. */
