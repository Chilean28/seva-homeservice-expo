...
export default function JobsScreen() {
  ...
  useEffect(() => {
    if (!user?.id) return;
    const channel = subscribeToWorkerJobs(user.id, () => {
      fetchJobs();
    });
    return () => {
      unsubscribe(channel);
  ...
  useEffect(() => {
    if (!workerId) return;
    const channel = subscribeToWorkerBookingUpdates(workerId, () => {
      fetchJobs();
    });
    return () => {
      unsubscribe(channel);
  ...
  return (
    <View style={styles.container}>
      ...
      >
        {loading ? (
          ...
          <Text style={styles.emptyText}>
            {tab === 'pending' && !workerId && 'Complete your profile to see and accept jobs'}
            {tab === 'pending' && workerId && 'No pending jobs'}
      ...
      <Modal visible={!!detailJob} transparent animationType="slide">
