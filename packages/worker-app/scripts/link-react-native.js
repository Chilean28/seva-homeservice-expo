 ...
 * packages/worker-app/node_modules/. Symlink any missing deps from root.
...
// Transitive deps that CocoaPods expects under ../node_modules (worker-app/node_modules)
...
if (linked > 0) {
  console.log('[worker-app] Linked', linked, 'hoisted package(s) from workspace root');
}

// Guard: never allow react/react-dom/react-native in worker-app/node_modules (causes duplicate React).
...
FORBIDDEN.forEach((pkg) => {
  ...
  try {
    ...
    if (stat.isSymbolicLink() || stat.isDirectory()) {
      console.warn(`[worker-app] Removing ${pkg} from node_modules (must resolve from workspace root)`);
