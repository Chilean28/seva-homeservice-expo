...
/** Use in signUp({ options: { emailRedirectTo } }) so confirmation emails open this app. */
export function getEmailConfirmationRedirectTo(): string {
  return Linking.createURL(AUTH_CALLBACK_PATH);
}

/**
 * True when the URL is our email-confirmation / OAuth return path (Expo dev or standalone scheme).
 */
export function isAuthCallbackDeepLink(url: string | null | undefined): boolean {
  if (!url) return false;
  return url.includes(AUTH_CALLBACK_PATH) && !url.includes('payment-success');
}

/**
 * Completes Supabase email confirmation (or magic-link) redirect: tokens in hash, or PKCE ?code=.
 */
