/**
 * Read-only tasker profile (e.g. from chat). No service selection or booking CTA.
 */
...
function subscriptionPrice(sub: WorkerDetail['service_subscriptions'][0]): number {
  const base = sub.services?.base_price ?? 0;
  return sub.custom_price != null ? Number(sub.custom_price) : Number(base);
}

export default function WorkerInfoOnlyScreen() {
  ...
  const minPrice =
    subs.length > 0 ? Math.min(...subs.map((s) => subscriptionPrice(s))) : null;
    ...
    <View style={styles.container}>
      ...
      >
        <View style={styles.summarySection}>
          ...
          <Text style={styles.serviceLabel}>{serviceSubtitle}</Text>
        ...
        {subs.length > 0 && (
          <View style={styles.section}>
            ...
            {subs.map((sub) => {
              ...
              const p = subscriptionPrice(sub);
              return (
                <View key={sub.service_id} style={styles.serviceRowStatic}>
...
const styles = StyleSheet.create({
  ...
  serviceLabel: { fontSize: 15, color: '#666', fontWeight: '600', marginBottom: 16, textAlign: 'center' },
  statsRow: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
  ...
  serviceOptionName: { fontSize: 16, fontWeight: '600', color: '#000', flex: 1 },
