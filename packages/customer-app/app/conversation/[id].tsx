...
export default function ConversationScreen() {
  const { user } = useAuth();
  const { refetchUnread } = useUnreadChat();
  ...
  }, [conversationId, user?.id, refetchUnread]);
  ...
  // Alert when linked booking status changes (worker accepts, completes, etc.)
  useEffect(() => {
    const bid = meta.bookingId;
    if (!bid) return;
    const channel = supabase
      .channel(`booking-status:${bid}`)
      .on(
        'postgres_changes',
        {
          event: 'UPDATE',
  ...
  const uploadAndSendImages = useCallback(
    async (uris: string[]) => {
      if (!conversationId || !user?.id || sending || !uris.length) return;
      setSending(true);
    ...
    [conversationId, user?.id, sending, input, fetchMessages]
  );
  ...
  const startRecording = useCallback(async () => {
    ...
    try {
      ...
      (rec as { setOnRecordingStatusUpdate: (cb: (s: { isRecording?: boolean; durationMillis?: number }) => void) => void }).setOnRecordingStatusUpdate((st) => {
        if (st.isRecording) setRecordingMillis(st.durationMillis ?? 0);
      });
      setRecording(rec);
    } catch {
  ...
  const stopRecordingAndSend = useCallback(async () => {
    ...
    try {
      ...
      if (st && typeof st.durationMillis === 'number' && st.durationMillis > 0) voiceDurationMs = st.durationMillis;
    } catch {
      /* keep recordingMillis */
    }
    try {
  ...
  const playVoiceMessage = useCallback(
    async (messageId: string, url: string) => {
      ...
      try {
        ...
        if (/native module|ExponentAV|cannot find/i.test(msg)) {
          Alert.alert('App update needed', 'Rebuild the app to play voice messages.');
        } else {
  ...
  const openWorkerProfile = useCallback(() => {
    if (meta.workerId) {
      router.push(`/worker/${meta.workerId}/info` as Parameters<typeof router.push>[0]);
    }
  }, [meta.workerId]);
  ...
  const bookingIdDisplay = meta.bookingId ? meta.bookingId.slice(-6) : null;
    ...
    <View style={styles.container}>
      ...
      <View style={[styles.headerWrapper, { paddingTop: insets.top }]}>
        <View style={styles.header}>
          ...
          {meta.avatarUrl ? (
            <Image source={{ uri: meta.avatarUrl }} style={styles.headerAvatar} resizeMode="cover" />
          ) : (
          ...
          <TouchableOpacity
            ...
            disabled={!meta.workerId}
            activeOpacity={meta.workerId ? 0.7 : 1}
          >
            <Text style={styles.headerTitle} numberOfLines={1}>{titleLine}</Text>
            {bookingIdDisplay ? (
              <Text style={styles.headerSubtitle}>Booking ID : {bookingIdDisplay} · Tap to view profile</Text>
            ) : meta.workerId ? (
            ...
            renderItem={({ item }) => {
              ...
              return (
                <View style={[styles.bubbleRow, own ? styles.bubbleRowOwn : styles.bubbleRowOther]}>
                  {!own ? (
                    meta.avatarUrl ? (
                      <Image source={{ uri: meta.avatarUrl }} style={styles.bubbleAvatar} resizeMode="cover" />
                    ) : (
...
const styles = StyleSheet.create({
  ...
  headerCenter: { flex: 1, minWidth: 0, justifyContent: 'center' },
  ...
  backButton: { marginTop: 16, paddingVertical: 12, paddingHorizontal: 24, backgroundColor: '#FFEB3B', borderRadius: 12 },
