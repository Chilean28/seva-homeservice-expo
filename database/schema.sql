...
-- Worker portfolio photos (past work, shown on public profile; ID document is not)
CREATE TABLE public.worker_portfolio_photos (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  worker_id UUID NOT NULL REFERENCES public.worker_profiles(id) ON DELETE CASCADE,
  photo_url TEXT NOT NULL,
...
CREATE INDEX idx_worker_portfolio_photos_worker_id ON public.worker_portfolio_photos(worker_id);
